...
// #2C5BFF, ink #0A0F1C, cream #F5F7FB.
...
export function emailShell(o: EmailShellOpts): string {
  const cta = o.cta
    ? `<div style="margin-top:22px"><a href="${o.cta.href}" style="display:inline-block;background:#2C5BFF;color:#ffffff;text-decoration:none;font-weight:600;font-size:15px;padding:12px 24px;border-radius:10px">${o.cta.text}</a></div>`
    : "";
  const intro = o.intro
  ...
  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head><body style="margin:0;background:#F5F7FB;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#0A0F1C">
  ...
  <tr><td align="center">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background:#ffffff;border:1px solid #E6E9F0;border-radius:16px;overflow:hidden">
      <tr><td style="padding:22px 28px;border-bottom:1px solid #EEF1F6">
        <span style="font-size:18px;font-weight:800;letter-spacing:-0.02em;color:#0A0F1C">Collab<span style="color:#2C5BFF">47</span></span>
      </td></tr>
      <tr><td style="padding:28px">
        <h1 style="margin:0 0 12px;font-size:20px;line-height:1.25;color:#0A0F1C">${o.title}</h1>
        ${intro}
        ${body}
        ${cta}
      </td></tr>
